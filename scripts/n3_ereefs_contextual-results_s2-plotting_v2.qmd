...
description: "This script describes methods to plot and visualise data extracted from the eReefs modelling suite."
author: "Adam Shand" 
format: html
---
...
The objective of this script is to produce plots of eReefs data. Each plot contains:
 ...
 - A line plot (smoothed using a GAM) that graphs daily values over time (daily value calculated from all cells)
 - A mean and median line across the defined time frame
...
Below we load in the data we want to plot. All datasets are stored within a single list.

```{r}

#list files in data folder that match the string
files_to_load <- list.files(glue("{output_path}/dry-tropics/"), pattern = "Chl", full.names = TRUE)

#load data
all_data <- map(files_to_load, read_stars)

```

# Plot Data

Then the plotting function can be used.
...
Below is a demonstration of creatin
