...
tm_shape(nc_chla_n3) + 
  tm_raster(col.scale = tm_scale_continuous(), 
            col.legend = tm_legend(reverse = T)) + 
  tm_layout(legend.outside = T) +
  tm_shape(n3_marine_region, is.main = T) + 
  tm_borders()
...
#cut to only DT area
dt_region <- n3_marine_region |> 
  filter(Region %in% c("Dry Tropics", "Burdekin"))
...
dt_map <- tm_shape(qld) +
  tm_polygons(fill = "grey80", 
              col = "black") +
  tm_shape(nc_chla_dt) + 
  tm_raster(col.scale = tm_scale_continuous(values = "brewer.bu_gn"), 
            col.legend = tm_legend(reverse = T)) + 
  tm_layout(legend.outside = T) +
...
tmap_save(dt_map, glue("{output_path}/dt_example_map.png"))
...
tm_shape(qld) +
  tm_polygons(fill = "grey80", 
              col = "black") +
  tm_shape(nc_chla_pre_cropped, is.main = T) + 
  tm_raster(col.scale = tm_scale_continuous(values = "brewer.bu_gn"),
            col.legend = tm_legend(reverse = T)) + 
  tm_layout(legend.outside = T)
