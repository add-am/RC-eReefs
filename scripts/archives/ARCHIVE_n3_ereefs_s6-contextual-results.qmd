---
title: "Creating Contextual Results for the Offshore Marine Zone"
...
#Note, to get params run "ctrl"+"shift"+"p"
...
#set the buffer around our target area
proj_buffer <- params$buffer
...
indicators_vect <- c("Chl_a_sum", "Secchi", "Turbidity", "DIN")

#get a bounding box of the marine region
marine_bbox <- st_bbox(st_buffer(target_marine_region, proj_buffer))
...
true_false_array <- x_grid >= box_bounds[1] & 
  x_grid <= box_bounds[2] & 
  y_grid >= box_bounds[3] & 
  y_grid <= box_bounds[4]
    ...
    if (i_lower == "secchi"){
      ...
      assign(glue("{i_lower}_{region_lower}"), 
             read_ncdf(input_file, var = i, 
                       ncsub = cbind(start = c(first_row, first_col, target_indicies[1, "Start"]),
                                     count = c(num_of_rows, num_of_cols, target_indicies[1, "Count"])),
      ...
      assign(glue("{i_lower}_{region_lower}"), 
             read_ncdf(input_file, var = i, 
                       ncsub = cbind(start = c(first_row, first_col,44, target_indicies[1, "Start"]),
                                     count = c(num_of_rows, num_of_cols, 1, target_indicies[1, "Count"])),
                       downsample = down_sample))
    }
...
names(data_list) <- c("Chla (ug/L)", "Secchi (m)", "Turbidity (NTU)", "DIN (mg[N]/m^3)")
...
convert_nc_to_sf <- function(dataset, group, sample_size = NULL){
  ...
  if (!is.null(sample_size)){#if we want a subset
    ...
    #create a function that picks randomly PER column
    sampled_data <- map(dataset_df, function(x){
    ...
    #bind this back into a dataframe
...
We will then convert data for three of the four indicators onto a log scale. Although the data has not be analysed statistically to "prove" this is a better distribution, a visual assessment of each indicators distribution of values was conducted to come to this conclusion. Histograms of values before and after are presented below.

Before:

```{r}
#| label: create histograms of data 1
#| output: TRUE

#create the histogram of the normal values
p_before <- ggplot(raw_data_df) +
  geom_histogram(aes(x = Values, fill = Indicator), bins = 150) +
...
```{r}
#| label: create histograms of data 2
#| output: TRUE

#convert values for three of four indicators to a log scale
raw_data_df_log <- raw_data_df |> 
  group_by() |> 
  mutate(Values = case_when(str_detect(Indicator, "Secchi") ~ Values,
                            T ~ log(Values))) |> 
  ungroup()

#shift all values back to zero by adding the min value per indicator
raw_data_df_log <- raw_data_df_log |> 
  group_by(Indicator) |> 
  mutate(Values = Values - min(Values)) |> 
  ungroup()

#create the histogram of the logged values
p_after <- ggplot(raw_data_df_log) +
  geom_histogram(aes(x = Values, fill = Indicator), bins = 150) +
  facet_wrap(~Indicator,
             scales = "free")
...
Its not perfect, but it certainly helps.

```{r}
#| label: finalise the log transformation

#rename the main dataset to be have the log values
raw_data_df <- raw_data_df_log

#repeat the log transformation for our subset of data
raw_data_df_subset <- raw_data_df_subset |> 
  group_by() |> 
  mutate(Values = case_when(str_detect(Indicator, "Secchi") ~ Values,
                            T ~ log(Values))) |> 
  ungroup()
...
#create a function that use either the linear or the log scale
args_per_indicator <- function(x){

  if (str_detect(names(monthly_mean_df)[[x]], "Secchi")){#if indicator is secchi use a linear scale
    
    tm_scale_continuous(values = indicator_palettes[x],
                         n = 6)
    
  } else {#otherwise, use a log scale
  
    tm_scale_continuous_log10(values = indicator_palettes[x],
                         n = 6)
...
map_name <- function(i){
  ...
  #get the name of the indicator currently being run
  ind_name <- names(data_list)[i]
  
  #replace the space with a new line
  ind_name <- str_replace(ind_name, " ", "\n")
  
  #if the indicator is not secchi, add the note that it is a log scale 
  ind_name <- if(!str_detect(ind_name, "Secchi")){glue("{ind_name} (log)")} else {ind_name}
  ...
  map <- tm_shape(qld) +
    ...
    tm_polygons(fill = "Value", 
                ...
                fill.legend = tm_legend(reverse = TRUE,
                                        title = map_name(i), #run our custom function from above
                                        na.show = FALSE), 
                col_alpha = 0) +
    tm_facets(by = "Month", 
              ncol = if(proj_region == "Wet Tropics"){6}else{4}) +
    tm_shape(reefs) +
