...
description: "A demonstration of downloading, saving, and reloading data extracted from the eReefs modelling suite."
author: "Adam Shand" 
format: html
---
...
Before diving in, some general script set up is needed.
...
#define a path to the input (data) and output folders
data_path <- here("data/")
...
my_sf_object <- st_read(glue("{data_path}/n3_region.gpkg")) |> 
...
multiple_extraction <- pmap(all_arguments,  function(Var1, Var2, Var3){ereefs_extract(my_sf_object, Var2, Var3, Var1)})
...
multiple_extraction <- map(multiple_extraction, ereefs_reproject)
...
#walk2(multiple_extraction, file_names, \(x, y) write_mdim(x, glue("{output_path}/{y}.nc")))

#build a list of files with the correct extension
#files_to_read <- list.files(glue("{output_path}"))[str_detect(list.files(glue("{output_path}")), ".nc")]

#read in all files
#mutiple_load <- map(files_to_read, \(x) read_stars(glue("{output_path}/{x}")))
...
dt_marine <- st_read(glue("{data_path}/n3_region.gpkg")) |> 
  filter(Region == "Dry Tropics", Environment == "Marine")
...
pwalk(all_arguments, function(Variable, StartDate, EndDate) {
  ...
  if (dim(data_extracted)[[3]] > 0){
    ...
    #create the folder to save each file in
    if (!dir.exists(glue("{output_path}/dry-tropics"))){dir.create(glue("{output_path}/dry-tropics"))}

    #build the file path
    file_path <- glue("{output_path}/dry-tropics/{Variable}_{StartDate}_{EndDate}.nc")
    
    #save
    write_mdim(data_reprojected, file_path)
...
#define the dry tropics region
wt_marine <- st_read(glue("{data_path}/n3_region.gpkg")) |> 
  filter(Region == "Wet Tropics", Environment == "Marine")

#use the same arguments table
...
#define the dry tropics region
mwi_marine <- st_read(glue("{data_path}/n3_region.gpkg")) |> 
